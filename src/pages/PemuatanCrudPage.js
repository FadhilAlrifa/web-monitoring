...
const PemuatanCrudPage = () => {
    ...
    const exportToCSV = () => {
        ...
        const headers = [
            "Tanggal", "Unit Kerja", "Target (Ton)", "Jam Muat (Jam)", "Ton Muat (Ton)"
        ];

        const csvRows = filteredLaporan.map(item => {
            const unitName = getUnitName(item.id_unit);

            // ---- FIX UTAMA: Gunakan tanggal asli (tanpa +1 / -1 hari) ---- //
        ...
        link.setAttribute(
            "download",
            `Laporan_Pemuatan_${PAGE_GROUP_NAME}_${new Date().toISOString().split('T')[0]}.csv`
        );
    ...
    const handleEdit = (laporanData) => {
        setFormData({
            ...
            jam_muat: parseFloat(laporanData.jam_muat) || 0, // Ambil jam_muat untuk payload PUT
        });
        setIsEditMode(true);
    ...
    const fetchLaporan = async () => {
        ...
        try {
            ...
            console.error('Error fetching pemuatan reports:', error.response?.data);
    ...
    const handleSubmit = async (e) => {
        ...
        // PAYLOAD INJEKSI: Memastikan jam_muat selalu 0 jika tidak diinput
        const payload = {
            ...formData,
            jam_muat: formData.jam_muat !== undefined ? formData.jam_muat : 0,
        ...
        <div className="min-h-screen bg-gray-50 p-4">
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-gray-800 mb-0 mt-0">Daftar Laporan Pemuatan</h2>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Jam Muat (Jam)</th>
